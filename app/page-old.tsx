...
export default function HomePage() {
  ...
  const handleRandomMeal = () => {
    // Filter out hidden dishes
    const availableSavory = dishAlternatives.savory.filter(dish => !isHidden(dish.name))
    const availableSoup = dishAlternatives.soup.filter(dish => !isHidden(dish.name))
    const availableVegetable = dishAlternatives.vegetable.filter(dish => !isHidden(dish.name))
    
    // Check if we have enough dishes
    if (availableSavory.length === 0 || availableSoup.length === 0 || availableVegetable.length === 0) {
      toast.error("Không đủ món ăn để chọn ngẫu nhiên", {
        description: "Hãy hiện lại một số món ăn trong Settings",
        duration: 4000,
      })
      return
    }
    
    const randomSavory = availableSavory[Math.floor(Math.random() * availableSavory.length)]
    const randomSoup = availableSoup[Math.floor(Math.random() * availableSoup.length)]
    const randomVegetable = availableVegetable[Math.floor(Math.random() * availableVegetable.length)]
      ...
      <main className="container mx-auto px-4 py-6 md:py-10 max-w-7xl">
        ...
        <section className="mb-12 md:mb-16">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 md:mb-6 gap-3">
            ...
            <div className="flex items-center gap-2 md:gap-3 px-4 md:px-5 py-2 md:py-3 bg-gradient-to-br from-card to-muted/50 rounded-xl md:rounded-2xl border border-border shadow-lg">
              ...
              <span className="text-base md:text-lg font-bold tabular-nums" suppressHydrationWarning>
                {isMounted ? currentTime.toLocaleTimeString("vi-VN", { 
                  hour: "2-digit", 
                  minute: "2-digit",
                  hour12: false 
                }) : "--:--"}
                          ...
                          <div className="p-4 pb-3 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  const availableDishes = dishAlternatives.savory.filter(dish => !isHidden(dish.name))
                                  if (availableDishes.length === 0) {
                                    toast.error("Không có món mặn nào để chọn", {
                                      description: "Hãy hiện lại một số món ăn trong Settings"
                                    })
                                    return
                                  }
                          ...
                          <div className="p-4 pb-3 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <Button
                                ...
                                className="gap-1.5 h-8"
                                onClick={() => {
                                  const availableDishes = dishAlternatives.soup.filter(dish => !isHidden(dish.name))
                                  if (availableDishes.length === 0) {
                                    toast.error("Không có món canh nào để chọn", {
                                      description: "Hãy hiện lại một số món ăn trong Settings"
                                    })
                                    return
                                  }
                          ...
                          <div className="p-4 pb-3 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <Button
                                ...
                                className="gap-1.5 h-8"
                                onClick={() => {
                                  const availableDishes = dishAlternatives.vegetable.filter(dish => !isHidden(dish.name))
                                  if (availableDishes.length === 0) {
                                    toast.error("Không có món rau nào để chọn", {
                                      description: "Hãy hiện lại một số món ăn trong Settings"
                                    })
                                    return
                                  }
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5 mb-12 md:mb-16">
          ...
          <Card className="overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-shadow bg-gradient-to-br from-card to-chart-3/5">
            <CardContent className="p-5 md:p-6">
              <div className="flex items-center justify-between mb-4 md:mb-5">
                <h3 className="font-bold text-base md:text-lg">Thành tích tuần</h3>
                <div className="h-9 w-9 md:h-11 md:w-11 rounded-lg bg-gradient-to-br from-chart-3 to-chart-3/70 flex items-center justify-center shadow-lg">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-3 bg-gradient-to-br from-chart-3/10 to-chart-3/5 rounded-lg">
                    <p className="text-xs text-muted-foreground mb-1">Món mới</p>
                    <p className="font-bold text-lg">3</p>
                </div>
                  <div className="text-center p-3 bg-gradient-to-br from-chart-2/10 to-chart-2/5 rounded-lg">
                    <p className="text-xs text-muted-foreground mb-1">Ngày hoàn thành</p>
                    <p className="font-bold text-lg">5/7</p>
                </div>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Mục tiêu calo:</span>
                    <span className="font-medium">1,400-1,600/ngày</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Ngân sách:</span>
                    <span className="font-medium">850k/1.2M VNĐ</span>
                  </div>
                </div>
                ...
                >
                  <TrendingUp className="mr-2 h-3.5 w-3.5 md:h-4 md:w-4" />
                  Xem chi tiết
                </Button>
                </Link>
              </div>
        ...
        <section className="mb-12 md:mb-16">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 md:mb-8 gap-3">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold mb-2">Kế hoạch tuần</h2>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-muted-foreground">18/21 bữa</span>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <Card className="bg-gradient-to-br from-chart-2/5 to-chart-2/10 border-chart-2/20">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-chart-2">📅 Tiến độ tuần</h3>
                    <div className="h-8 w-8 rounded-lg bg-chart-2/20 flex items-center justify-center">
                      <Calendar className="h-4 w-4 text-chart-2" />
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">Đã lên kế hoạch</span>
                      <span className="font-bold text-lg">18/21</span>
                    </div>
                    ...
                    <Link href="/weekly-plan" className="w-full">
              ...
              <Card>
                <CardContent className="p-4">
                  <h3 className="font-semibold mb-2">💡 Mẹo lên kế hoạch</h3>
                  <ul className="text-sm text-muted-foreground space-y-1">
                    <li>• Lên kế hoạch vào cuối tuần</li>
                    <li>• Chuẩn bị nguyên liệu trước</li>
                    <li>• Đa dạng món ăn mỗi ngày</li>
              ...
              <Card>
                <CardContent className="p-4">
                  <h3 className="font-semibold mb-2">📊 Thống kê tuần</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>Bữa đã lên kế hoạch:</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 auto-rows-fr">
            <Link href="/menu" className="h-full">
              <motion.div whileHover={{ y: -8, scale: 1.02 }} transition={{ duration: 0.3 }} className="h-full">
                ...
                <div className="absolute top-3 right-3 md:top-4 md:right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  <HideButton
                    itemId="vegetarian-menu-7days"
                    itemName="Thực đơn chay thanh đạm"
                    itemType="meal"
                    itemImage="/healthy-salad-bowl-colorful-vegetables.jpg"
                    size="sm"
                    variant="ghost"
            ...
            <Link href="/dishes" className="h-full">
              <motion.div whileHover={{ y: -8, scale: 1.02 }} transition={{ duration: 0.3, delay: 0.1 }} className="h-full">
                ...
                <div className="absolute top-3 right-3 md:top-4 md:right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  <HideButton
                    itemId="tofu-mushroom-trending"
                    itemName="Đậu hũ sốt nấm hương"
                    itemType="meal"
                    itemImage="/grilled-chicken-rice-asian-meal.jpg"
                    size="sm"
                    variant="ghost"
                    className="bg-white/90 hover:bg-white text-gray-700 hover:text-gray-900"
                  />
            ...
            <motion.div whileHover={{ y: -8, scale: 1.02 }} transition={{ duration: 0.3, delay: 0.2 }} className="h-full">
              ...
              >
                ...
                <div className="absolute top-3 right-3 md:top-4 md:right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  <HideButton
                    itemId="vegetarian-restaurant-nearby"
                    itemName="Quán ăn chay gần bạn"
                    itemType="restaurant"
                    itemImage="/restaurant-interior-cozy-dining.jpg"
                    size="sm"
                    variant="ghost"
                    className="bg-white/90 hover:bg-white text-gray-700 hover:text-gray-900"
                  />
        ...
        <section className="mb-12 md:mb-16">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 md:mb-8 gap-3">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold mb-2">Tính năng nổi bật</h2>
              <p className="text-sm md:text-base text-muted-foreground flex items-center gap-2">
                <Sparkles className="h-3.5 w-3.5 md:h-4 md:w-4 text-primary" />
                Khám phá các công cụ hữu ích cho ẩm thực
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <Link href="/cook/dau-hu-sot-ca" className="group">
              <Card className="overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 h-full">
                <CardContent className="p-6 text-center">
                  <div className="h-12 w-12 mx-auto mb-4 rounded-xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                    <ChefHat className="h-6 w-6 text-white" />
                  </div>
                  <h3 className="font-bold text-lg mb-2">Công thức nấu ăn</h3>
                  <p className="text-sm text-muted-foreground mb-4">Hướng dẫn chi tiết từng bước</p>
            ...
            <Link href="/shopping" className="group">
              <Card className="overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 h-full">
                <CardContent className="p-6 text-center">
                  <div className="h-12 w-12 mx-auto mb-4 rounded-xl bg-gradient-to-br from-chart-1 to-chart-1/70 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                    <ShoppingCart className="h-6 w-6 text-white" />
                  </div>
                  <h3 className="font-bold text-lg mb-2">Danh sách mua sắm</h3>
                  <p className="text-sm text-muted-foreground mb-4">Tự động gom nguyên liệu</p>
                  <Badge variant="secondary" className="text-xs">Thông minh</Badge>
                </CardContent>
              </Card>
            </Link>

            <Link href="/weekly-plan" className="group">
              <Card className="overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 h-full">
                <CardContent className="p-6 text-center">
                  <div className="h-12 w-12 mx-auto mb-4 rounded-xl bg-gradient-to-br from-chart-2 to-chart-2/70 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                    <Calendar className="h-6 w-6 text-white" />
                  </div>
                  <h3 className="font-bold text-lg mb-2">Kế hoạch tuần</h3>
                  <p className="text-sm text-muted-foreground mb-4">Lên lịch bữa ăn 7 ngày</p>
                  <Badge variant="secondary" className="text-xs">86% hoàn thành</Badge>
                </CardContent>
              </Card>
            </Link>
            ...
            <Link href="/settings" className="group">
              <Card className="overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1 h-full">
                <CardContent className="p-6 text-center">
                  <div className="h-12 w-12 mx-auto mb-4 rounded-xl bg-gradient-to-br from-chart-3 to-chart-3/70 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                    <Settings className="h-6 w-6 text-white" />
                  </div>
                  <h3 className="font-bold text-lg mb-2">Cài đặt cá nhân</h3>
                  <p className="text-sm text-muted-foreground mb-4">Tùy chỉnh sở thích ăn uống</p>
                  <Badge variant="secondary" className="text-xs">20+ tùy chọn</Badge>
                </CardContent>
              </Card>
            </Link>
          </div>
